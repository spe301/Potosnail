...
class MachineLearning:
    ...
    @classmethod    
    def SeeModel(self, cls, model, parameters, data, target, task, ensemble=False):
...
class DeepLearning:
    ...
    def DeepTabularRegression(self, base, activation, loss, optimizer):
        '''Builds a FeedForward net that does regression on tabular data'''
        output_dim = 1
        oa = 'linear'
        model = models.Sequential()
        model.add(layers.Dense(base, activation=activation))
        model.add(layers.Dense(int(base/2), activation=activation))
        model.add(layers.Dense(int(base/4), activation=activation))
        model.add(layers.Dense(output_dim, activation=oa))
        model.compile(loss=loss, optimizer=optimizer)
        return model
    
    def DeepTabularClassification(self, output_dim, base, activation, loss, optimizer):
    ...
    def RNN(self, nodes, activation, optimizer, dropout, regularizer, method, stacking, bidirectional, inp, nclasses):
        ...
        if method == 'LSTM':
          ...
          if regularizer == 'L1':
            if bidirectional == False:
              model.add(layers.LSTM(nodes, activation=activation, kernel_regularizer=Regularizer.l1(0.005), return_sequences=stacking))
            else:
              model.add(layers.Bidirectional(layers.LSTM(nodes)))
          if regularizer == 'L2':
            if bidirectional == False:
              model.add(layers.LSTM(nodes, activation=activation, kernel_regularizer=Regularizer.L2(0.005), return_sequences=stacking))
            else:
              model.add(layers.Bidirectional(layers.LSTM(nodes)))
        if method == 'GRU':
          if regularizer == None:
          ...
          if regularizer == 'L1':
            if bidirectional == False:
              model.add(layers.GRU(nodes, activation=activation, kernel_regularizer=Regularizer.l1(0.005), return_sequences=stacking))
            else:
              model.add(layers.Bidirectional(layers.GRU(nodes)))
          if regularizer == 'L2':
            if bidirectional == False:
              model.add(layers.GRU(nodes, activation=activation, kernel_regularizer=Regularizer.L2(0.005), return_sequences=stacking))
            else:
              model.add(layers.Bidirectional(layers.GRU(nodes)))
        if dropout == True:
    ...
    def BuildCNN(self, data, task, start=32, optimizer='sgd', filter_size=(3,3), pool=(2,2), epochs=30, batch_size=32, vizualize=False):
        ...
        target_size = (data[0].shape[1], data[0].shape[2])
        model = models.Sequential()
        model.add(layers.Conv2D(start, filter_size, activation='relu', input_shape=(target_size[0], target_size[1], 3)))
        model.add(layers.MaxPooling2D(pool))
        model.add(layers.Conv2D(start, filter_size, activation='relu'))
        model.add(layers.MaxPooling2D(pool))
        model.add(layers.Conv2D(start*2, filter_size))
        model.add(layers.MaxPooling2D(pool))
        model.add(layers.Flatten())
        model.add(layers.Dense(start*2, activation='relu'))
        model.add(layers.Dense(output_dim, activation=act))
        model.compile(optimizer=optimizer, loss=loss, metrics=['accuracy'])
        return model
    ...
    def CollectPerformance(self, params, func, X, y, epochs=50, batch_size=32, patience=5):
                      ...
                      for i in range(len(params[n[7]])):
                        ...
                        for i in range(len(params[n[8]])):
                          ...
                          for i in range(len(params[n[9]])):
                            ...
                            epics.append(len(history.history['accuracy']))
    ...
    def ModelReadyPixles(self, directories, classes, method, target_size=(150, 150)):
        '''gets images ready to be fed into a CNN'''
        if method == 'Image Classification':
            ...
            for i in range(2, n):
                ...
                if n != 2:
                    ...
                    tray = np.reshape(trl, (len(trl), 1))
                    tey = np.reshape(tel, (len(tel), 1))
...
class DataHelper:
    ...
    def MakeDirs(self, train_dir, test_dir, classes, method):
